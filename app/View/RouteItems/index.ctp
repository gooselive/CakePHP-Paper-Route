<div class="routeItems index">
	<h2><?php echo __('Route Items'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('route_id'); ?></th>
			<th><?php echo $this->Paginator->sort('direction_id'); ?></th>
			<th><?php echo $this->Paginator->sort('address_id'); ?></th>
			<th><?php echo $this->Paginator->sort('route_order'); ?></th>
			<th><?php echo $this->Paginator->sort('created'); ?></th>
			<th><?php echo $this->Paginator->sort('modified'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php
	foreach ($routeItems as $routeItem): ?>
	<tr>
		<td><?php echo h($routeItem['RouteItem']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($routeItem['RouteItem']['route_id'], array('controller' => 'routeItems', 'action' => 'view', $routeItem['RouteItem']['route_id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($routeItem['Direction']['direction'], array('controller' => 'directions', 'action' => 'view', $routeItem['Direction']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($routeItem['Address']['address'], array('controller' => 'addresses', 'action' => 'view', $routeItem['Address']['id'])); ?>
		</td>
		<td><?php echo h($routeItem['RouteItem']['route_order']); ?>&nbsp;</td>
		<td><?php echo h($routeItem['RouteItem']['created']); ?>&nbsp;</td>
		<td><?php echo h($routeItem['RouteItem']['modified']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $routeItem['RouteItem']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $routeItem['RouteItem']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $routeItem['RouteItem']['id']), null, __('Are you sure you want to delete # %s?', $routeItem['RouteItem']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>

	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Route Item'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Routes'), array('controller' => 'routes', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Route'), array('controller' => 'routes', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Directions'), array('controller' => 'directions', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Direction'), array('controller' => 'directions', 'action' => 'add')); ?> </li>
	</ul>
</div>
