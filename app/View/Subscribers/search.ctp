<?php echo $this->element('searchForm/set_pagination_named');//set pagination named element ?>
<div class="subscribers index">
	
	<h2><?php echo __('Subscribers'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('address'); ?></th>
			<th><?php echo $this->Paginator->sort('name'); ?></th>
			<th><?php echo $this->Paginator->sort('spouse_name'); ?></th>
			<th><?php echo $this->Paginator->sort('phone'); ?></th>
			<th><?php echo $this->Paginator->sort('cell_phone'); ?></th>
			<th><?php echo $this->Paginator->sort('work_phone'); ?></th>
			<th><?php echo $this->Paginator->sort('email'); ?></th>
			<th><?php echo $this->Paginator->sort('employer'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php	foreach ($subscribers as $subscriber): ?>
	<tr>
		<td><?php echo ucwords(strtolower($this->Html->link($subscriber['Address']['address'], array('controller' => 'addresses', 'action' => 'view', $subscriber['Address']['id'])))); ?></td>
		<td><?php echo h($subscriber['Subscriber']['name']); ?>&nbsp;</td>
		<td><?php echo h($subscriber['Subscriber']['spouse_name']); ?>&nbsp;</td>
		<td><?php echo h($subscriber['Subscriber']['phone']); ?>&nbsp;</td>
		<td><?php echo h($subscriber['Subscriber']['cell_phone']); ?>&nbsp;</td>
		<td><?php echo h($subscriber['Subscriber']['work_phone']); ?>&nbsp;</td>
		<td><?php echo h($subscriber['Subscriber']['employer']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $subscriber['Subscriber']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $subscriber['Subscriber']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $subscriber['Subscriber']['id']), null, __('Are you sure you want to delete # %s?', $subscriber['Subscriber']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>

	<div class="paging">
	<?php
		echo $this->Paginator->first('<< ' . __('first'), array());
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
		echo $this->Paginator->last('>> ' . __('last'), array());
	?>
	</div>

</div>
<div class="actions">
	<?php echo $this->element('searchForm/subscriberform') //search form element?>
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('Search Addresses'), array('controller' => 'addresses','action' => 'search')); ?></li>
		<li><?php echo $this->Html->link(__('New Address'), array('controller' => 'addresses','action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Gifts'), array('controller' => 'gifts', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Gift'), array('controller' => 'gifts', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('Search Subscriptions'), array('controller' => 'subscriptions', 'action' => 'search')); ?> </li>
		<li><?php echo $this->Html->link(__('New Subscription'), array('controller' => 'subscriptions', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('New Subscriber'), array('action' => 'add')); ?> </li>
	</ul>

</div>

